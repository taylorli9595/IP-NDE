function Is=multi_div4(In,Id, q,sigma,alpha,lamba,k)
I=double(In);
Id=double(Id)+50*randn(size(Id));
...
term1=bx./termx;
term11=term1.*Iss;
term2=by./(k*lamba*(bys.^2+minmod(bxs,fxs).^2).^(alpha/2)+k*(1-lamba)*(byd.^2+minmod(bxd,fxs).^2).^(alpha/2)+1);
term22=term2.*Iss;
